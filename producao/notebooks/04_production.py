...
FLUXO:
  1. Carrega dados âncora (abt00_test.csv)
  2. Aplica transformações do notebook de tratamento 
  3. Seleciona features do arquivo features_stage_02.pkl
  4. Carrega e aplica pipeline do modelo (scaler + LogisticRegression)
  5. Gera predições e salva resultados

ALTERAÇÃO: Mude apenas a variável DATA_ENTRADA para processar novos dados
"""
...
from configs.paths import (
    PROCESSED_DIR, ARTIFACT_DIR, PREDICTIONS_DIR, 
...
TARGET = 'FPD'
THRESHOLD = 0.41
VERSAO = 'V1-1 - RL(Sklearn SEM Premissas)'
...
def carregar_artefatos():
    ...
    # Pipeline do modelo
    pipe = joblib.load(MODELS_DIR / 'logreg_pipeline.pkl')
    print('✓ Pipeline do modelo carregado')
...
def preparar_dados(df, features, stats):
    ...
    - Trata valores especiais ('Desconhecido')
    - Aplica cap de segurança (SCORE_RATEO)
    - Converte para float64
    - Preenche valores nulos com estatísticas do treino
    - Seleciona features corretas
    """
    print('\n🔄 Processando dados...')
    ...
    cols_desconhecido = [c for c in df_proc.columns 
                         if df_proc[c].astype(str).eq('Desconhecido').any()]
    for col in cols_desconhecido:
        df_proc[col] = df_proc[col].astype(str).str.strip().replace('Desconhecido', np.nan)
    
    # Segurança: cap no SCORE_RATEO
    if 'SCORE_RATEO' in df_proc.columns:
        df_proc['SCORE_RATEO'] = df_proc['SCORE_RATEO'].replace([np.inf, -np.inf], np.nan).clip(-10, 10)
    
    # Converte para float
    df_proc = df_proc.astype('float64')
