...
def plotar_feature_importance(modelo, feature_names, top_n=20):
    ...
    # Mapeia índices de features para nomes reais
    ...
    arquivo = METRICS_DIR / f'feature_importance_{timestamp}.png'
...
def plotar_curva_roc(y_true, proba):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    arquivo = METRICS_DIR / f'roc_curve_{timestamp}.png'
    plt.savefig(arquivo, bbox_inches='tight')
    print(f'✓ Salvo: {arquivo}')
    plt.close()
...
def plotar_distribuicao_scores(y_true, proba, threshold=THRESHOLD):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    arquivo = METRICS_DIR / f'score_distribution_{timestamp}.png'
    plt.savefig(arquivo, bbox_inches='tight')
    print(f'✓ Salvo: {arquivo}')
    plt.close()
...
def plotar_confusion_matrix(y_true, pred):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    arquivo = METRICS_DIR / f'confusion_matrix_{timestamp}.png'
    plt.savefig(arquivo, bbox_inches='tight')
    print(f'✓ Salvo: {arquivo}')
    plt.close()
...
def plotar_ks_plot(y_true, proba):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    arquivo = METRICS_DIR / f'ks_plot_{timestamp}.png'
    plt.savefig(arquivo, bbox_inches='tight')
    print(f'✓ Salvo: {arquivo}')
    plt.close()
...
def plotar_precision_recall_curve(y_true, proba):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    arquivo = METRICS_DIR / f'precision_recall_curve_{timestamp}.png'
    plt.savefig(arquivo, bbox_inches='tight')
    print(f'✓ Salvo: {arquivo}')
    plt.close()
