...
def salvar_resultados(df_original, proba, pred):
    ...
    arquivo_pred = PREDICTIONS_DIR / f'predicoes_xgb_{timestamp}.csv'
...
def plotar_feature_importance(modelo, feature_names, top_n=20):
    ...
    # Salva gráfico
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    arquivo_plot = METRICS_DIR / f'feature_importance_xgb_{timestamp}.png'
